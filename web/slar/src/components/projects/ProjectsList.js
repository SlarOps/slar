...
export default function ProjectsList({ projects, loading, onManageMembers, onDeleteProject }) {
  if (loading) {
    return (
...
function ProjectCard({ project, onManageMembers, onDeleteProject }) {
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-3 text-sm text-gray-500 dark:text-gray-400">
            ...
            {project.incident_count !== undefined && (
              <div className="flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <span>{project.incident_count} incidents</span>
              </div>
            )}
            {project.user_role && (
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getRoleBadgeColor(project.user_role)}`}>
                {project.user_role}
              </span>
            )}
          </div>
          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-20">
                <button
                  onClick={() => {
                    window.location.href = `/projects/${project.id}/incidents`;
                    setShowMenu(false);
                  }}
