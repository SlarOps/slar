  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-900">
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          {messages.map((m, idx) => (
            <div key={idx} className={`mb-6 ${m.role === "user" ? "text-right" : "text-left"}`}>
                  ...
                  <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-blue-800 dark:text-blue-200">
                        Knowledge Sources {m.originalContent && Array.isArray(m.originalContent) ? `(${m.originalContent.filter(item => item.metadata).length})` : ''}
                      </span>
                    </div>

                    {m.originalContent && Array.isArray(m.originalContent) && (
                      <div className="space-y-2">
                        {m.originalContent.filter(item => item.metadata).map((item, index) => (
              ...
              {Array.isArray(m.incidents) && m.incidents.length > 0 && (
      ...
      <ChatInput
        ...
        onRemoveAttachment={() => setAttachedIncident(null)}
